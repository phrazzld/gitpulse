...
import { setCacheItem, getStaleItem, ClientCacheTTL } from '@/lib/localStorageCache'
...
export function useInstallations(options: UseInstallationsOptions) {
  ...
  useEffect(() => {
    ...
    const { data: cachedCurrentInstallations } =
      getStaleItem<Installation[]>('currentInstallations')
  ...
  const setInstallations = useCallback(
    (newInstallations: Installation[] | readonly Installation[]) => {
   ...
   */
  const switchInstallations = useCallback(
    (installIds: number[]) => {
