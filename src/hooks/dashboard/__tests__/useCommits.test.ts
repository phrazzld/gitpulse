...
describe('useCommits', () => {
  ...
  it('should fetch commits successfully', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining(
        '/api/summary?since=2022-01-01&until=2022-12-31&groupBy=chronological'
      )
    )
  })
  ...
  it('should use correct API endpoint based on activity mode', async () => {
    ...
    ;(createActivityFetcher as jest.Mock).mockReturnValue(() =>
      Promise.resolve({
        data: [],
        nextCursor: null,
        hasMore: false,
  ...
  it('should include filter parameters in the request', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(expect.stringMatching(/since=2022-01-01/))
    expect(global.fetch).toHaveBeenCalledWith(expect.stringMatching(/until=2022-12-31/))
    expect(global.fetch).toHaveBeenCalledWith(expect.stringMatching(/organizations=org1%2Corg2/))
    expect(global.fetch).toHaveBeenCalledWith(expect.stringMatching(/repositories=repo1%2Crepo2/))
    expect(global.fetch).toHaveBeenCalledWith(expect.stringMatching(/contributors=user1%2Cuser2/))
    expect(global.fetch).toHaveBeenCalledWith(expect.stringMatching(/installation_ids=123%2C456/))
  })
})
