 ...
 */
export async function fetchAllRepositoriesOAuth(accessToken: string): Promise<Repository[]> {
  ...
  try {
    ...
    const combinedRepos = await octokit.paginate(octokit.rest.repos.listForAuthenticatedUser, {
      per_page: 100,
      sort: 'updated',
      visibility: 'all',
    ...
    try {
      ...
      for (const org of orgs) {
        try {
          ...
          logger.warn(MODULE_NAME, `Error fetching repos for org: ${org.login}`, {
            error: orgError,
          })
        }
      }
