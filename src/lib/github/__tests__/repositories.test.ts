...
// Import Jest globals for proper TypeScript types
...
import { logger } from '../../logger' // Fixed path
...
class MockOctokit {
  ...
  withRateLimitError(): MockOctokit {
    this.rest.rateLimit.get = jest.fn().mockRejectedValue(new Error('Rate limit error'))
    return this
  }

  withMissingRepoScope(): MockOctokit {
  ...
  withMissingReadOrgScope(): MockOctokit {
    this.rest.users.getAuthenticated = jest.fn().mockResolvedValue({
      data: { login: 'testuser', id: 123, type: 'User' },
  ...
  withUserInfoError(): MockOctokit {
    this.rest.users.getAuthenticated = jest.fn().mockRejectedValue(new Error('User info error'))
    return this
  }

  withOrganizations(orgs = sampleOrgs): MockOctokit {
    this.rest.orgs.listForAuthenticatedUser = jest.fn()
    this.paginate.mockImplementation((method: any, options: any) => {
  ...
  withOrgReposError(orgName: string): MockOctokit {
  ...
  withMixedRepos(): MockOctokit {
    ...
    // Org list
    originalPaginate.mockImplementationOnce(() => Promise.resolve(sampleOrgs))

    // First org repos
    originalPaginate.mockImplementationOnce(() =>
      Promise.resolve([{ id: 3, name: 'repo3', full_name: 'org1/repo3', private: false }])
    )
  ...
  withEmptyResponse(): MockOctokit {
    this.paginate = jest.fn().mockResolvedValue([])
    return this
  }

  withNonArrayResponse(): MockOctokit {
    ...
    originalPaginate.mockImplementationOnce(() => Promise.resolve([...sampleRepositories]))

    // Org list returns single org
    originalPaginate.mockImplementationOnce(() => Promise.resolve([{ login: 'org1', id: 101 }]))
...
jest.mock('octokit', () => {
  const mockOctokitConstructor = jest.fn((options?: any) => {
...
describe('GitHub Repositories Module', () => {
  ...
  describe('fetchAllRepositoriesOAuth', () => {
    ...
    it('should warn when rate limit is running low', async () => {
      ...
      mockOctokit.mockImplementationOnce(() => {
        return new MockOctokit().withLowRateLimit() as unknown as Octokit
      })
    ...
    it('should handle rate limit check errors', async () => {
      ...
      const mockOctokit = jest.mocked(Octokit)
      mockOctokit.mockImplementationOnce(() => {
        return new MockOctokit().withRateLimitError() as unknown as Octokit
      })
    ...
    it('should warn when token lacks repo scope', async () => {
      ...
      const mockInstance = new MockOctokit().withMissingRepoScope()

      // Reset the mock constructor to return our configured mock
      const octokitMock = jest.mocked(Octokit)
      octokitMock.mockImplementationOnce(() => mockInstance as unknown as Octokit)
    ...
    it('should warn but not error when token lacks read:org scope', async () => {
      ...
      const mockOctokit = jest.mocked(Octokit)
      mockOctokit.mockImplementationOnce(() => {
        return new MockOctokit().withMissingReadOrgScope() as unknown as Octokit
      })
    ...
    it('should handle user info fetch errors', async () => {
      ...
      const mockOctokit = jest.mocked(Octokit)
      mockOctokit.mockImplementationOnce(() => {
        return new MockOctokit().withUserInfoError() as unknown as Octokit
      })
    ...
    it('should handle empty scopes header', async () => {
      ...
      const mockInstance = new MockOctokit()
      mockInstance.rest.users.getAuthenticated = jest.fn().mockResolvedValue({
        data: { login: 'testuser', id: 123, type: 'User' },
    ...
    it('should fetch and include organization repositories', async () => {
      ...
      const mockOctokit = jest.mocked(Octokit)
      mockOctokit.mockImplementationOnce(() => {
        return new MockOctokit().withMixedRepos() as unknown as Octokit
      })
    ...
    it('should handle errors when fetching org repositories', async () => {
      ...
      mockOctokit.mockImplementationOnce(() => {
        ...
        return mock.withOrgReposError('org1') as unknown as Octokit
      })
    ...
    it('should handle non-array response from organization repo API', async () => {
      ...
      mockOctokit.mockImplementationOnce(() => {
        return new MockOctokit()
          ...
          .withNonArrayResponse() as unknown as Octokit
      })
    ...
    it('should handle errors when listing organizations', async () => {
      ...
      const mockOctokit = jest.mocked(Octokit)
      mockOctokit.mockImplementationOnce(() => {
        return new MockOctokit().withOrgListError() as unknown as Octokit
      })
    ...
    it('should deduplicate repositories by full_name', async () => {
      ...
      const mockOctokit = jest.mocked(Octokit)
      mockOctokit.mockImplementationOnce(() => {
        return new MockOctokit().withDuplicateRepos() as unknown as Octokit
      })
    ...
    it('should propagate errors from the main API call', async () => {
      ...
      const mockOctokit = jest.mocked(Octokit)
      mockOctokit.mockImplementationOnce(() => {
        return new MockOctokit().withApiError() as unknown as Octokit
      })
  ...
  describe('fetchAllRepositoriesApp', () => {
    ...
    it('should handle installation auth errors', async () => {
      ...
      ;(getInstallationOctokit as jest.Mock).mockRejectedValue(new Error('Invalid installation'))
  ...
  describe('fetchAllRepositories', () => {
    ...
    it('should propagate errors from fetchAllRepositoriesApp', async () => {
      ...
      ;(getInstallationOctokit as jest.Mock).mockRejectedValue(new Error('App installation error'))
