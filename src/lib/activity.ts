  ...
  return commits.map((commit, index) => {
    try {
      ...
      if (commit.commit && typeof commit.commit === 'object') {
        ...
        commitMessage =
          typeof commit.commit.message === 'string' && commit.commit.message.trim() !== ''
            ? commit.commit.message
            : 'No commit message available'

        // Get author from commit.commit.author (or fallback)
        const commitAuthor =
          commit.commit.author && typeof commit.commit.author === 'object'
            ? commit.commit.author
            : null

        authorName =
          commitAuthor && typeof commitAuthor.name === 'string' && commitAuthor.name.trim() !== ''
            ? commitAuthor.name
            : 'Unknown Author'

        authorDate =
          commitAuthor && typeof commitAuthor.date === 'string' && commitAuthor.date.trim() !== ''
            ? commitAuthor.date
      ...
      // Handle MinimalCommit shape from optimize.ts (message, author_name, author_date)
        ...
        authorName =
          typeof commit.author_name === 'string' && commit.author_name.trim() !== ''
            ? commit.author_name
        ...
        authorDate =
          typeof commit.author_date === 'string' && commit.author_date.trim() !== ''
            ? commit.author_date
      ...
      if (commit.repository && typeof commit.repository === 'object') {
        const repoName =
          typeof commit.repository.name === 'string' ? commit.repository.name : 'Unknown Repo'
        const repoFullName =
          typeof commit.repository.full_name === 'string'
            ? commit.repository.full_name
            : `unknown/${repoName}`
        const repoUrl =
          typeof commit.repository.html_url === 'string' ? commit.repository.html_url : undefined
      ...
      if (commit.contributor && typeof commit.contributor === 'object') {
        contributor = {
          username:
            typeof commit.contributor.username === 'string'
              ? commit.contributor.username
              : 'unknown',
          displayName:
            typeof commit.contributor.displayName === 'string'
              ? commit.contributor.displayName
              : 'Unknown User',
          avatarUrl:
            typeof commit.contributor.avatarUrl === 'string' ? commit.contributor.avatarUrl : null,
        }
      }
      // GitHub-style author object
      else if (commit.author && typeof commit.author === 'object') {
        contributor = {
          username: typeof commit.author.login === 'string' ? commit.author.login : 'unknown',
          displayName:
            typeof commit.author.name === 'string'
              ...
              : typeof commit.author.login === 'string'
                ? commit.author.login
                : 'Unknown User',
          avatarUrl: typeof commit.author.avatar_url === 'string' ? commit.author.avatar_url : null,
        }
      }
      // Minimal author fields (author_login, author_avatar)
      else if (
        typeof commit.author_login === 'string' ||
        typeof commit.author_avatar === 'string'
      ) {
        contributor = {
          username:
            typeof commit.author_login === 'string' && commit.author_login.trim() !== ''
              ? commit.author_login
              : 'unknown',
          displayName: authorName, // Use the already extracted author name
          avatarUrl: typeof commit.author_avatar === 'string' ? commit.author_avatar : null,
        }
      }
  ...
  return async (cursor: string | null, limit: number) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Network error: ${getSafeErrorMessage(networkError, 'Failed to connect to server')}`
        )
      }
