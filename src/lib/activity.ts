...
export function transformApiCommit(commit: unknown): InternalActivityCommit {
  ...
  return {
    ...
    commit: {
      ...
      author: {
        name:
          ((commitData?.author as Record<string, unknown> | null)
    ...
    repository: repoData
      ? {
          ...
          fullName:
            (repoData.fullName as string) ||
            (repoData.full_name as string) ||
            "",
          htmlUrl:
            (repoData.htmlUrl as string) || (repoData.html_url as string),
        }
      : undefined,
    // contributor field removed for individual-only focus
  };
 ...
 */
export function prepareActivityCommit(
  internalCommit: InternalActivityCommit,
): ActivityCommit {
  return {
...
export function createActivityFetcher(
  ...
  return async (
    ...
    const paginationData = data.pagination as
      ...
      | undefined;
    const hasMore =
      (paginationData?.hasMore as boolean) ||
      (paginationData?.has_more as boolean) ||
      false;
    const nextCursor =
      (paginationData?.nextCursor as string) ||
      (paginationData?.next_cursor as string) ||
      null;
