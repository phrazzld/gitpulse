...
import { AppInstallation, Repository, Commit } from '@/lib/github/types'
...
import { FilterInfo, GroupBy, GroupedResult, CommitStats, SummaryResponse } from '@/types/api'
...
export function filterRepositoriesByOrgAndRepoNames(
  ...
  if (repositoryFilters.length > 0) {
    filteredRepos = filteredRepos.filter(repo => repositoryFilters.includes(repo.full_name))
...
export async function fetchCommitsWithAuthMethod(
  ...
  for (const [key, repos] of Object.entries(reposByInstallation)) {
    ...
    if (key === 'oauth') {
      ...
      commitFetchPromises.push(
        fetchCommitsForRepositories(accessToken, installId, repos, since, until, authorFilter)
      )
    }
  }
...
export function prepareSummaryResponse(
  ...
  return {
    ...
    currentInstallations:
      installationIds.length > 0 ? installations.filter(i => installationIds.includes(i.id)) : [],
  }
}
