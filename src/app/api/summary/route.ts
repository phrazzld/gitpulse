...
import { getAllAppInstallations, fetchAllRepositories, AppInstallation } from '@/lib/github'
...
export async function GET(request: NextRequest) {
  ...
  if (!since || !until) {
    ...
    return new NextResponse(
      JSON.stringify({ error: 'Missing required parameters: since and until dates' }),
      {
        status: 400,
        headers: {
          'Content-Type': 'application/json',
  ...
  try {
    ...
    if (session.accessToken) {
      try {
        ...
        if (installationIds.length > 0 && allInstallations.length > 0) {
          ...
          const invalidIds = installationIds.filter(id => !validInstallationIds.includes(id))
    ...
    if (installationIds.length > 0) {
      ...
      const repoPromises = installationIds.map(id => fetchAllRepositories(session.accessToken, id))
    ...
    const filteredRepos = filterRepositoriesByOrgAndRepoNames(
      allRepos,
      organizations,
      repositoryFilters
    )
    ...
    const filteredCommits = filterCommitsByContributor(
      commits,
      contributors,
