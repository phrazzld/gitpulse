...
import { Commit, Repository, AppInstallation } from '@/lib/github/types'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { pagedCommits, hasMore, nextCursor } = applyPagination(optimizedCommits, cursor, limit)

    // Extract stats
    const stats = {
      totalCommits: allCommits.length,
      repositories: Array.from(
        new Set(allCommits.map(commit => commit.repository?.full_name || ''))
      ),
      dates: Array.from(
        new Set(allCommits.map(commit => commit.commit.author?.date?.split('T')[0] || ''))
      ),
      organizations,
      contributors,
    }
