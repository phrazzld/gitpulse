  ...
  paginate: jest.fn().mockImplementation((method) => {
    ...
    if (method === mockOctokit.rest.repos.listForOrg) {
      return Promise.resolve(
        mockRepositories.filter((r) => r.owner.login === "testorg"),
      );
    }
...
export const mockCreateAuthenticatedOctokit = jest
  .fn()
...
export const mockFetchAppRepositories = jest
  .fn()
  .mockImplementation((octokit) => {
...
export const mockFetchRepositoryCommitsWithOctokit = jest
  .fn()
  .mockImplementation((octokit, owner, repo, since, until, author) => {
    if (!octokit) {
      return Promise.reject(new Error("Octokit instance is required"));
    }
    ...
    return Promise.resolve(
      mockActivityCommits.map((commit) => ({
        ...commit,
        repository: {
...
export const mockFetchCommitsForRepositoriesWithOctokit = jest
  .fn()
  .mockImplementation((octokit, repositories, since, until, author) => {
    if (!octokit) {
      return Promise.reject(new Error("Octokit instance is required"));
    }
    return Promise.resolve(mockActivityCommits);
...
export const mockFetchAllRepositories = jest
  .fn()
  .mockImplementation((accessToken, installationId) => {
...
export const mockFetchRepositoryCommits = jest
  .fn()
  .mockImplementation(
    (accessToken, installationId, owner, repo, since, until, author) => {
      // Validate at least one authentication method is provided
      if (!accessToken && !installationId) {
        return Promise.reject(
          new Error(
...
export const mockFetchCommitsForRepositories = jest
  .fn()
  .mockImplementation(
    (accessToken, installationId, repositories, since, until, author) => {
      // Validate at least one authentication method is provided
      if (!accessToken && !installationId) {
        return Promise.reject(
          new Error(
...
export const mockGetServerSession = getServerSession as jest.Mock &
  import("@/types/testing").TypedMockFunction<unknown>;
...
jest.mock("@/lib/auth/tokenValidator", () => ({
  ...
  useAuthValidator: jest
    .fn()
    .mockReturnValue({ isValidating: false, isValid: true }),
}));
...
jest.mock("@/lib/auth/apiAuth", () => {
  ...
  const withAuthValidation: import("@/types/api").ApiMiddleware = (handler) => {
    return async (req) => {
...
export const mockErrors = {
  ...
  createConfigError: () =>
    new GitHubConfigError("GitHub App not properly configured", {
      context: { functionName: "testFunction" },
    }),

  // Auth errors (status 403)
  createAuthError: () =>
    new GitHubAuthError("GitHub authentication failed", {
      status: 403,
      context: { functionName: "testFunction" },
    }),
  createTokenError: () =>
    new GitHubAuthError("GitHub token is invalid or expired", {
      status: 403,
      context: { functionName: "testFunction" },
    }),
  createScopeError: () =>
    new GitHubAuthError("GitHub token is missing required scope", {
      status: 403,
      context: { functionName: "testFunction" },
    }),
  ...
  createNotFoundError: () =>
    new GitHubNotFoundError("Repository not found", {
      context: { functionName: "testFunction" },
    }),

  // API error (variable status)
  createApiError: (status = 500) =>
    new GitHubApiError("GitHub API operation failed", {
      status,
      context: { functionName: "testFunction" },
    }),
  ...
  createGitHubError: () =>
    new GitHubError("Generic GitHub error", {
      context: { functionName: "testFunction" },
    }),
...
export const verifyOctokitPassing = (
  ...
  expect(fetchFunction).toHaveBeenCalledWith(mockOctokit, ...additionalArgs);
