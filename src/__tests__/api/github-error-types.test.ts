...
const verifyStandardizedErrorResponse = (
  response: import("@/types/testing").ApiTestResponse,
  expected: {
    statusCode: number;
    errorCode: string;
  ...
  if (
    expected.errorMessageContains &&
    expected.errorMessageContains.length > 0
  ) {
    expected.errorMessageContains.forEach((fragment) => {
      expect(typeof response.data.error).toBe("string");
      // Use a case-insensitive check for more resilient tests
      expect((response.data.error as string).toLowerCase()).toContain(
...
const reposTestHelper = createApiHandlerTestHelper(
  getRepos as import("@/types/api").ApiRouteHandler,
);
const myActivityTestHelper = createApiHandlerTestHelper(
  getMyActivity as import("@/types/api").ApiRouteHandler,
);
const summaryTestHelper = createApiHandlerTestHelper(
  getSummary as import("@/types/api").ApiRouteHandler,
);
...
describe("API Routes: GitHub Error Type Handling", () => {
  ...
  describe("Error Response Structure Uniformity", () => {
    ...
    it("ensures standardized error responses for each error type", async () => {
      ...
      const verifyStandardErrorStructure = (
        response: import("@/types/testing").ApiTestResponse,
      ) => {
      ...
      mockFetchAppRepositories.mockRejectedValueOnce(
        mockErrors.createRateLimitError(),
      );
      ...
      mockFetchAppRepositories.mockRejectedValueOnce(
        mockErrors.createNotFoundError(),
      );
