...
describe('API Error Handling', () => {
  describe('createApiErrorResponse', () => {
    ...
    test('should handle GitHubRateLimitError and return 429 with resetAt timestamp', async () => {
      ...
      const error = new MockGitHubRateLimitError("GitHub API rate limit exceeded", {
        status: 429,
        resetTimestamp,
        context: { functionName: "testFunction" }
      });
    ...
    test('should handle GitHubConfigError and return 500', async () => {
      // Arrange
      const error = new MockGitHubConfigError("GitHub App not properly configured", {
        context: { functionName: "testFunction" }
      });
  ...
  describe('withErrorHandling HOF', () => {
    ...
    test('should pass through successful responses', async () => {
      // Arrange
      const wrappedHandler = mockWithErrorHandling(mockSuccessHandler, 'test-module');
    ...
    test('should properly handle GitHubAuthError', async () => {
      // Arrange
      const error = new MockGitHubAuthError("GitHub authentication failed", { status: 403 });
    ...
    test('should pass handler arguments to the handler', async () => {
      // Arrange
      const handlerWithArgs = jest.fn().mockResolvedValue(MockNextResponse.json({ success: true }));
      const wrappedHandler = mockWithErrorHandling(handlerWithArgs, 'test-module');
