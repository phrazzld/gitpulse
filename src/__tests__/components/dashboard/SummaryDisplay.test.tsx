...
describe("SummaryDisplay", () => {
  ...
  it("returns null when summary is null", () => {
    const { container } = render(
      <SummaryDisplay summary={null} {...defaultProps} />,
    );
  ...
  it("renders with minimal summary data (without AI summary)", () => {
    ...
    render(<SummaryDisplay summary={minimalSummary} {...defaultProps} />);
    ...
    if (!commitCountSection)
      throw new Error("Could not find commit count section");
    ...
    expect(
      screen.queryByText("COMPREHENSIVE ANALYSIS"),
    ).not.toBeInTheDocument();
  });

  it("renders with full summary data including AI summary", () => {
    render(<SummaryDisplay summary={mockSummary} {...defaultProps} />);
    ...
    const patternsSection = screen
      .getByText("IDENTIFIED PATTERNS")
      .closest(".mb-8");
    ...
    mockSummary.aiSummary.keyThemes.forEach((theme) => {
      expect(
        within(patternsSection as HTMLElement).getByText(theme),
      ).toBeInTheDocument();
    });

    // Find the technical areas section
    const technicalAreasSection = screen
      .getByText("TECHNICAL FOCUS AREAS")
      .closest(".mb-8");
    if (!technicalAreasSection)
      throw new Error("Could not find technical areas section");

    // Check for specific technical areas without asserting by count
    mockSummary.aiSummary.technicalAreas.forEach((area) => {
      expect(
        within(technicalAreasSection as HTMLElement).getByText(area.name),
      ).toBeInTheDocument();

      // Find the containing element for this technical area within the section
      const areaElement = within(technicalAreasSection as HTMLElement)
        .getByText(area.name)
        .closest(".flex");
      if (!areaElement)
        throw new Error(
          `Could not find container element for technical area "${area.name}"`,
        );
      expect(areaElement).toHaveTextContent(area.count.toString());
    });

    // Find the accomplishments section
    const accomplishmentsSection = screen
      .getByText("KEY ACHIEVEMENTS")
      .closest(".mb-8");
    if (!accomplishmentsSection)
      throw new Error("Could not find achievements section");

    // Check for specific accomplishments
    mockSummary.aiSummary.accomplishments.forEach((accomplishment) => {
      expect(
        within(accomplishmentsSection as HTMLElement).getByText(accomplishment),
      ).toBeInTheDocument();
    });

    // Find the commit classification section
    const commitClassificationSection = screen
      .getByText("COMMIT CLASSIFICATION")
      .closest(".mb-8");
    if (!commitClassificationSection)
      throw new Error("Could not find commit classification section");

    // Check for commit types without direct count matching
    mockSummary.aiSummary.commitsByType.forEach((type) => {
      expect(
        within(commitClassificationSection as HTMLElement).getByText(type.type),
      ).toBeInTheDocument();
      expect(
        within(commitClassificationSection as HTMLElement).getByText(
          type.description,
        ),
      ).toBeInTheDocument();
      // Find the container element for this commit type
      const typeElement = within(commitClassificationSection as HTMLElement)
        .getByText(type.type)
        .closest(".border-l-2");
      if (!typeElement)
        throw new Error(
          `Could not find container element for commit type "${type.type}"`,
        );
      expect(typeElement).toHaveTextContent(type.count.toString());
    });

    // Find the comprehensive analysis section
    const analysisDiv = screen
    ...
    if (!analysisSection)
      throw new Error("Could not find analysis section parent element");

    // Check for overall summary
    expect(
      within(analysisSection as HTMLElement).getByText(
        mockSummary.aiSummary.overallSummary,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("configures ActivityFeed correctly", () => {
    ...
    expect(emptyMessageElement.textContent).toContain(
  ...
  it("handles repositories filter parameters when provided", () => {
    render(
      <SummaryDisplay
        summary={mockSummary}
        activityMode="my-activity"
    ...
    // Activity feed would include repository filters if they were implemented
