...
export class MockNextResponse {
  ...
  private _body: import("@/types/testing").MockResponseBody;

  constructor(
    body: import("@/types/testing").MockResponseBody,
    options: {
      status?: number;
      statusText?: string;
  ...
  static json(
    data: import("@/types/testing").MockResponseData,
...
export class MockGitHubError extends Error {
  ...
  public readonly context?: import("@/types/testing").ErrorContext;
...
export const mockErrors = {
  ...
  createConfigError: () =>
    new MockGitHubConfigError("GitHub App not properly configured", {
      context: { functionName: "testFunction" },
    }),

  // Auth errors (status 403)
  createAuthError: () =>
    new MockGitHubAuthError("GitHub authentication failed", {
      status: 403,
      context: { functionName: "testFunction" },
    }),
  createTokenError: () =>
    new MockGitHubAuthError("GitHub token is invalid or expired", {
      status: 403,
      context: { functionName: "testFunction" },
    }),
  createScopeError: () =>
    new MockGitHubAuthError("GitHub token is missing required scope", {
      status: 403,
      context: { functionName: "testFunction" },
    }),
  ...
  createRateLimitError: () =>
    new MockGitHubRateLimitError("GitHub API rate limit exceeded", {
      status: 429,
  ...
  createNotFoundError: () =>
    new MockGitHubNotFoundError("Repository not found", {
      context: { functionName: "testFunction" },
    }),

  // API error (variable status)
  createApiError: (status = 500) =>
    new MockGitHubApiError("GitHub API operation failed", {
      status,
      context: { functionName: "testFunction" },
    }),
  ...
  createGitHubError: () =>
    new MockGitHubError("Generic GitHub error", {
      context: { functionName: "testFunction" },
    }),
