...
export default function FilterPanel({
  ...
  const [groupBy, setGroupBy] = useState<
    'contributor' | 'organization' | 'repository' | 'chronological'
  >('chronological')
  ...
  const fetchContributors = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/contributors?organizations=${orgList}&include_commit_count=true`
      )
      if (!response.ok) {
  ...
  useEffect(() => {
    if (
      expanded &&
      !loadingContributors &&
      (contributors.length === 0 || shouldFetchContributors())
    ) {
      fetchContributors()
    }
  }, [
    ...
    >
      ...
      {expanded && (
        <div className="mt-4 space-y-6">
          {/* Contributors filter */}
          <div>
            <h4 className="text-xs mb-2 font-bold" style={{ color: 'var(--electric-blue)' }}>
              CONTRIBUTOR FILTER
            </h4>
            <div className="space-y-2">
              ...
              <div className="flex items-center">
                ...
                />
                <label htmlFor="only-me" className="text-sm" style={{ color: 'var(--foreground)' }}>
              ...
              {!selectOnlyMe && (
                <div className="pl-5 pt-2">
                  ...
                  {loadingContributors ? (
                    ...
                    >
                      <span
                        className="inline-block w-3 h-3 border-2 border-t-transparent rounded-full animate-spin mr-2"
                      ...
                      {contributors.map(contributor => (
                        <div key={contributor.username} className="flex items-center mb-2">
                          <input
                            ...
                            onChange={e =>
                              handleContributorChange(contributor.username, e.target.checked)
                            }
                            disabled={isLoading}
                            className="mr-2"
                            style={{
