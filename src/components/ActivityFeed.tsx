    ...
    if (canTriggerInfiniteScroll && hasMore && !loading) {
      ...
      loadMore()
        .catch(loadError => {
          ...
          logger.error(MODULE_NAME, 'Error while loading more items in intersection handler', {
            loadError,
          })
        })
        .finally(() => {
        ...
        <div className="relative w-full">
          ...
          <div className="flex justify-center p-2">
            <div className="text-xs flex items-center" style={{ color: 'var(--neon-green)' }}>
              <span
                className="inline-block w-3 h-3 mr-2 border-2 border-t-transparent rounded-full animate-spin"
        ...
        <div className="relative" ref={listContainerRef}>
          ...
          >
            ...
            {listWidth > 0 && (
              <List
                height={
                  typeof calculateListHeight() === 'string'
                    ? windowWidth * 0.7
                    : (calculateListHeight() as number)
                }
                width={listWidth}
