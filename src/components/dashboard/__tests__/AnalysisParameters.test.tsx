...
const render = (component: any) => {
  return {
    getByText: (text: string): MockElement => {
      ...
      if (
        component.props.activityMode &&
        text === getActivityDisplayText(component.props.activityMode)
      ) {
        return { textContent: text }
      }

      if (
        component.props.dateRange &&
        text === `${component.props.dateRange.since} to ${component.props.dateRange.until}`
      ) {
        return { textContent: text }
      }

      if (
        component.props.organizations?.length > 0 &&
        text === `${component.props.organizations.length} SELECTED`
      ) {
        return { textContent: text }
      }
      ...
      if (
        ...
        text === 'DATE RANGE' ||
        text === 'ORGANIZATIONS' ||
        (component.props.showHelpText !== false &&
          text ===
