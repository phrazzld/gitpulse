...
import { checkColorContrast, ContrastResult } from '../../src/lib/accessibility/colorContrast';
 ...
 */
export function parseCSSVariables(cssContent: string): CSSVariables {
  const variables: CSSVariables = { dark: {} };
 ...
 */
function resolveVariable(value: string, variables: Record<string, string>): string {
 ...
 */
export function validateColorPairings(config: Config, cssVariables: CSSVariables): ValidationResult[] {
 ...
 */
export async function generateColorDocs(validateOnly = false): Promise<void> {
  try {
    // Read CSS file
