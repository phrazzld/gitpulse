const { detectStagedStoryFiles, runAccessibilityCheck, parseViolations } = require('../check-a11y-staged-stories');
...
describe('check-a11y-staged-stories', () => {
  ...
  describe('main function', () => {
    test('should exit early when no story files are staged', () => {
      ...
      expect(mockLog).toHaveBeenCalledWith('No staged Storybook files to check for accessibility.');
      expect(mockExit).toHaveBeenCalledWith(0);
    });
  });
});